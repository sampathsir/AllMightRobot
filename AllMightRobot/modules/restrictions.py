...
async def kick_user_cmd(message, chat, user, args, strings):
    ...
    if user_id == BOT_ID:
        await message.reply(strings['kick_AllMight'])
        return
...
async def mute_user_cmd(message, chat, user, args, strings):
    ...
    if user_id == BOT_ID:
        await message.reply(strings['mute_AllMight'])
        return
...
async def unmute_user_cmd(message, chat, user, strings):
    ...
    if user_id == BOT_ID:
        await message.reply(strings['unmute_AllMight'])
        return
...
async def ban_user_cmd(message, chat, user, args, strings):
    ...
    if user_id == BOT_ID:
        await message.reply(strings['ban_AllMight'])
        return
...
async def unban_user_cmd(message, chat, user, strings):
    ...
    if user_id == BOT_ID:
        await message.reply(strings['unban_AllMight'])
        return
...
<b>Examples:</b>
<code>- Mutes a user for two hours.
